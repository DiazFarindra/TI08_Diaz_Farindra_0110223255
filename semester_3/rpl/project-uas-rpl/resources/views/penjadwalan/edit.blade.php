...
<body class="bg-gray-100">
    ...
    <form action="{{ route('penjadwalan.update', $penjadwalan->id) }}" method="post" enctype="multipart/form-data" class="w-full max-w-4xl mx-auto mt-10 p-8 bg-white shadow-lg rounded-lg">
        @csrf
        @method('PATCH')
        ...
        <div class="mb-6">
            ...
            <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $penjadwalan->tanggal) }}"
        ...
        <div class="mb-6">
            ...
            <input type="text" id="jam" name="jam" value="{{ old('jam', $penjadwalan->jam) }}"
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        ...
        <div class="mb-6">
            ...
            <input type="text" id="topik" name="topik" value="{{ old('topik', $penjadwalan->topik) }}"
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        ...
        <div class="mb-6">
            ...
            <select id="status" name="status"
                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                <option value="terlaksana" {{ old('status', $penjadwalan->status) == 'terlaksana' ? 'selected' : '' }}>Terlaksana</option>
                <option value="tidak_terlaksana" {{ old('status', $penjadwalan->status) == 'tidak_terlaksana' ? 'selected' : '' }}>Tidak Terlaksana</option>
            </select>
        </div>
