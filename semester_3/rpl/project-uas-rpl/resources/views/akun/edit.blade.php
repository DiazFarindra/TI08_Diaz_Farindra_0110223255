    ...
    <form action="{{ route('akun.update', $akun->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-lg mt-8">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nim" name="nim" value="{{ old('nim', $akun->nim) }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
            </div>
            ...
            <!-- No Telpon -->
            <div class="mb-4">
                <label for="telpone" class="block text-sm font-medium text-gray-700">No Telpon</label>
                <input type="text" id="telpone" name="telpone" value="{{ old('telpone', $akun->telpone) }}"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="peran" name="peran"
                    ...
                    <option value="mahasiswa" {{ old('peran', $akun->peran) == 'mahasiswa' ? 'selected' : '' }}>Mahasiswa</option>
                    <option value="dosen" {{ old('peran', $akun->peran) == 'dosen' ? 'selected' : '' }}>Dosen</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="status" name="status"
                    ...
                    <option value="tidak_aktif" {{ old('status', $akun->status) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            </div>
